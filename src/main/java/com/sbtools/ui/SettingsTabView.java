...
public class SettingsTabView extends VBox {
    ...
    public SettingsTabView(SettingsStore settingsStore, LicenseValidator licenseValidator) {
        ...
        getChildren().add(createSection("License", createLicenseSection(settings)));
    ...
    private VBox createLicenseSection(AppSettings s) {
        licenseStatusLabel = new Label();
        licenseStatusLabel.setWrapText(true);
        licenseStatusLabel.setStyle("-fx-font-size: 13px;");

        LicenseValidator.LicenseStatus status = licenseValidator.check();
        applyLicenseStatus(status);

        Button changeLicenseBtn = new Button("Change License");
        changeLicenseBtn.getStyleClass().addAll("button", "button-outlined");
        changeLicenseBtn.setOnAction(e -> {
            LicenseDialog dialog = new LicenseDialog();
            dialog.show(licenseValidator);
            licenseValidator.check();
            refreshLicenseStatus();
        });

        VBox box = new VBox(8);
        box.getChildren().addAll(licenseStatusLabel, changeLicenseBtn);
        return box;
    }

    private void applyLicenseStatus(LicenseValidator.LicenseStatus status) {
        var type = status.type();
        if (type == LicenseValidator.LicenseStatus.StatusType.ACTIVE) {
            licenseStatusLabel.setText("Status: Active - expires " + status.expiryDate().format(DATE_FMT)
                    + " (" + status.daysRemaining() + " days)");
            licenseStatusLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #50fa7b;");
        } else if (type == LicenseValidator.LicenseStatus.StatusType.EXPIRED) {
            licenseStatusLabel.setText("Status: Expired on " + status.expiryDate().format(DATE_FMT));
            licenseStatusLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #ff5555;");
        } else if (type == LicenseValidator.LicenseStatus.StatusType.NO_LICENSE) {
            licenseStatusLabel.setText("Status: No license (Free version)");
            licenseStatusLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #ffb86c;");
        } else {
            licenseStatusLabel.setText("Status: Unknown");
            licenseStatusLabel.setStyle("-fx-font-size: 13px;");
        }
    }
