...
public class BrowserTracesCleaner implements CleanerExtension {
    ...
    private static final java.util.Map<String, String> BROWSER_PROCESS_MAP = java.util.Map.of(
            "Chrome", "chrome.exe",
            "Edge", "msedge.exe",
            "Firefox", "firefox.exe",
            "Brave", "brave.exe",
            ...
            if (!browserRunning) {
                for (Path f : extraFiles) {
                    if (Files.isRegularFile(f)) {
                        try { long size = Files.size(f); CleanerUtils.deletePermanently(f); cleaned += size; } catch (Exception ignored) {}
                    }
                }
            }
        }
    ...
    private String getBrowserProcessKey(String profileName) {
        for (String key : BROWSER_PROCESS_MAP.keySet()) {
            if (profileName.startsWith(key) || profileName.equals(key)) return key;
        }
        return null;
    }
