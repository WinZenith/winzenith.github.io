...
# ── GPU ──────────────────────────────────────────────────────────────────────
# Get-CimInstance deserializes AdapterRAM as Int32, truncating VRAM > 4 GB.
# Use [wmi] which returns the raw UInt32 value we can cast to UInt64.
$gpus = @()
...
foreach ($gpuObj in $gpuSearcher.Get()) {
    ...
    # AdapterRAM via [wmi] returns UInt32; cast to UInt64
    $vramBytes = [uint64]0
    try {
        $raw = $vc['AdapterRAM']
    ...
    # For NVIDIA GPUs, prefer nvidia-smi which reports actual VRAM correctly
    # (WMI AdapterRAM is UInt32 and truncates VRAM > 4GB)
    $gpuName = ''
    ...
    # Determine memory type from VideoMemoryType
    $memoryType = ''
    try {
    ...
    # Fallback: infer from GPU name if still empty
    if ($memoryType -eq '') {
        $nameLower = ''
...
# Helper function to resolve DDR type from SMBIOSMemoryType value
function Get-DdrType {
    param([int]$smbiosType, [string]$memoryTypeCim)
    # SMBIOSMemoryType values (SMBIOS spec):
    # 0x01=Other, 0x02=Unknown, 0x03=DRAM, 0x04=EDRAM, 0x05=VRAM
    # 0x06=SRAM, 0x07=RAM, 0x08=ROM, 0x09=Flash, 0x0A=EEPROM
    # 0x0B=FEPROM, 0x0C=EPROM, 0x0D=CDRAM, 0x0E=3DRAM
    # 0x0F=SDRAM, 0x10=SGRAM, 0x11=RDRAM, 0x12=DDR, 0x13=DDR2
    # 0x14=DDR2 FB-DIMM, 0x18=DDR3, 0x19=FBD2, 0x1A=DDR4, 0x1B=LPDDR
    # 0x1C=LPDDR2, 0x1D=LPDDR3, 0x1E=LPDDR4, 0x1F=Logical Non-Volatile DRAM
    ...
    # If SMBIOSMemoryType is 0 or 2 (Unknown), try CIM MemoryType as fallback
...
foreach ($memObj in $memSearcher.Get()) {
    ...
    # SMBIOSMemoryType for accurate DDR type detection
    $smbiosMemType = 0
    try { $raw = $mem['SMBIOSMemoryType']; if ($null -ne $raw) { $smbiosMemType = [int]$raw } } catch {}

    # CIM MemoryType as fallback
    $cimMemType = 'Unknown'
    try {
        $raw = $mem['MemoryType']
        if ($null -ne $raw) {
...
# Check for NVMe drives via Win32_PhysicalMedia
$nvmes = @()
...
# Detect chipset via PnP devices (Intel/AMD PCH/Southbridge)
$chipset = ''
$southbridge = ''
...
# Also check system devices for chipset name
if (-not $chipset) {
...
# Try registry for chipset info
if (-not $chipset) {
    try {
        $regBase = 'HKLM:\HARDWARE\DESCRIPTION\System\BIOS'
        $regChipset = Get-ItemPropertyValue -Path $regBase -Name 'BaseBoardProduct' -ErrorAction SilentlyContinue
        # Not reliable for chipset, but try another path
        $pnpReg = Get-ChildItem 'HKLM:\SYSTEM\CurrentControlSet\Enum\PCI' -ErrorAction SilentlyContinue |
            Where-Object { $_.PSChildName -match '8086|1022' } | Select-Object -First 3
    } catch {}
}
# Final fallback: parse from BIOS/system info
if (-not $chipset) {
    try {
...
Get-CimInstance Win32_PnPEntity | Where-Object { $_.DeviceID -and $_.Name -ne 'Unknown device' } | ForEach-Object {
    ...
    # Also filter by PNPClass for devices whose DeviceID doesn't match patterns above
    $pnpClassRaw = ''
